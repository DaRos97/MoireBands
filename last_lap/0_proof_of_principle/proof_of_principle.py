...
#Model parameters
type_of_stacking = 'P' #or 'AP'
m1,m2,mu = (0.13533,0.53226,-1.16385)
a,b,c = (-0.04,-0.03,-0.3) if type_of_stacking=='P' else (0,-0.03,-0.3)
f1,f2 = (1,1) if type_of_stacking=='P' else (1,2)
V,phi = (0.01,np.pi)
if 0:
    #N=a=b=f1=f2=0
    #V=phi=0
...
title = "Type of stacking: "+type_of_stacking+', parameters\n'+'a: '+"{:.3f}".format(a)+'b: '+"{:.3f}".format(b)+'c: '+"{:.3f}".format(c)+'\n'+'f1: '+"{:.3f}".format(f1)+'f2: '+"{:.3f}".format(f2)
print(title)
#
#Extract S11 image
...
if not Path(en_fn).is_file() or not Path(wg_fn).is_file():
    ...
    for i in tqdm(range(k_pts)):
        ...
        for e in range(n_cells):
            for l in range(2):
                weights[i,e+l*n_cells] += np.abs(evecs[n_cells*l,e])**2
    if save:
