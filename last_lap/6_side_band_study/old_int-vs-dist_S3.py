...
plt.xticks([0,len_e//2,len_e],["{:.2f}".format(E_f),"{:.2f}".format((E_i+E_f)/2),"{:.2f}".format(E_i)])
...
plt.xticks([0,len_k//2,len_k],["{:.2f}".format(K_i),"{:.2f}".format((K_i+K_f)/2),"{:.2f}".format(K_f)])
plt.xlabel(r"$\mathring{A}^{-1}$",size=s_)
plt.yticks([0,len_e//2,len_e],["{:.2f}".format(E_f),"{:.2f}".format((E_i+E_f)/2),"{:.2f}".format(E_i)])
plt.ylabel("eV",size=s_)

YYYY = 375
plt.hlines(YYYY,0,len_k,color='r')
print("energy offset: "+"{:.2f}".format(E_f+YYYY/len_e*(E_i-E_f))+" eV")
