...
def plot_bands(*args,figname='',show=False,title=''):
    pic,K_list,hopping,epsilon,HSO,par_offset,best_interlayer_pars,sample,monolayer_type,k_pts,EM,Em = args
    #plot
    ...
    ens = {}
    #
    for int_type in label.keys():
        energies = energy(K_list,hopping,epsilon,HSO,par_offset,best_interlayer_pars[sample][monolayer_type][int_type],int_type)
    ...
    ax.set_yticks([0,pic.shape[0]//2,pic.shape[0]],["{:.2f}".format(EM),"{:.2f}".format((EM+Em)/2),"{:.2f}".format(Em)],size=25)
...
def get_fit_pars_fn(TMD,machine):
...
def get_res_fn(title,int_type,machine):
    return get_home_dn(machine)+'Data/'+title+'_'+int_type+'_pars_interlayer.npy'

def get_home_dn(machine):
    if machine == 'loc':
        return '/home/dario/Desktop/git/MoireBands/Code/2_interlayer/'
    elif machine == 'hpc':
        return '/home/users/r/rossid/2_interlayer/'
    elif machine == 'maf':
        print("Mafalda not implemented")
        exit()
