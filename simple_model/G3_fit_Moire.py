...
E_max_fig = -0.5
K_lim = 0.5
E_i = 20   #relevant window: initial pixel from top
E_f = pic.shape[0]*2//3-20 #relevant window: final pixel from top
if 0: #cut old image to get just new image without axes
    new_pic = np.array(np.asarray(im)[E_i:E_f,:])
...
E_min = E_min_fig + E_f*delta_E/temp
E_max = E_max_fig - E_i*delta_E/temp
pic = np.array(np.asarray(im)[E_i:E_f,:])
#save cut image
new_image = Image.fromarray(np.uint8(pic))
new_imagename = dirname + "G_cut_data_Moire.png"
new_image.save(new_imagename)
...
path = fs.path_BZ_small(a_mono[0],len_K//k_points_factor,K_lim)     #K-points in BZ cut K-G-K' with modulus between -lim and lim
